...
class drsn_tdcnn(_TDCNN):
    def __init__(self, depth=34, pretrained=False):
        self.model_path = '/home/tmy/3D-ResNets-PyTorch-master/data/hmdb51/results/save_200.pth'
        self.pretrained = pretrained
        self.depth = depth
        self.shortcut_type = 'A' if depth in [18, 34] else 'B'
        self.dout_base_model = 256 if depth in [18, 34] else 1024
        #self.model_path = 'data/pretrained_model/resnet-34-kinetics.pth'
        # self.model_path = '/home/tmy/3D-ResNets-PyTorch-master/data/hmdb51/results/save_200.pth'.format(depth)
        self.dout_top_model = 512 if depth in [18, 34] else 2048
        _TDCNN.__init__(self)

    def _init_modules(self):
        net_str = "drsn{}(sample_size=112, sample_duration={}, shortcut_type=\'{}\')".format(self.depth, int(cfg.TRAIN.LENGTH[0]), self.shortcut_type)
        drsn = eval(net_str)
        if self.pretrained:
            print("Loading pretrained weights from %s" %(self.model_path))
            state_dict = torch.load(self.model_path)['state_dict']
            # parallel unpack (model = nn.DataParallel(model, device_ids=None))
            drsn.load_state_dict({k[7:] : v for k,v in state_dict.items() if k[7:] in drsn.state_dict()}, strict=False)
